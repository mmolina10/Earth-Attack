...
function Update(){	
	...
	if(verificador == false){
    	...
    	transform.localEulerAngles.x =  h*30; 
   		...
   		if((-Input.acceleration.y) && (-Input.acceleration.x) || (-Input.acceleration.y) && (Input.acceleration.x) || (Input.acceleration.y) && (-Input.acceleration.x) || (Input.acceleration.y) && (Input.acceleration.x)){
   			verificador = true;
		}
		else{
			verificador = false;
		...
		if(verificador == false){
			transform.Rotate(0,Input.acceleration.x * velocidadCamara * Time.deltaTime, 0);
		}
	#endif
}
